...
# Start with H3K4me1, exclude experiments that didn't run
cemt_table_H3K4me1 <- cemt_table %>% filter(mark == "H3K4me1") %>% 
    filter(!(individual_number %in% as.character(c(8)) & celltype %in% "LC")) %>% 
    filter(!(individual_number %in% as.character(c(8)) & celltype %in% "LP")) %>% 
    filter(!(individual_number %in% as.character(c(8)) & celltype %in% "SC"))
# Modules folder path
modules_path <- "/projects/epigenomics3/epigenomics3_results/users/ahauduc/arp/project/modules"
# Initialize results tibble
ase_depths_H3K4me1 <- tibble()

for (i in seq_along(cemt_table_H3K4me1$cemt)) {
    # Import ChIP-seq treatment vcf file
    ase_treatment_vcf <- 
        readVcfAsVRanges(str_c(modules_path, "/notebook/alignment_analysis/ase/output/", cemt_table_H3K4me1$mark[i], "_", cemt_table_H3K4me1$celltype[i], "_", "Individual", "_", cemt_table_H3K4me1$individual_number[i], "_treatment_cis_hbtls.vcf.gz")) %>%
        as.data.frame() %>%
        select(-width, -strand) %>%
    ...
    ase_het_positions <-
        join_overlap_intersect(consensus.all.spaas.rpkm.bed %>% 
                                   plyranges::filter(mark == cemt_table_H3K4me1$mark[i], celltype == cemt_table_H3K4me1$celltype[i], spaa == TRUE) %>% 
                                   keepSeqlevels(const_canonical_chromosomes, pruning.mode = "coarse"),
                               readVcfAsVRanges(str_c(modules_path, "/1/output2/", cemt_table_H3K4me1$mark[i], "/", cemt_table_H3K4me1$cemt[i], "/wgs/", cemt_table_H3K4me1$cemt[i], ".recal_hard_filtered_phased_snvs.vcf.gz")) %>%
                                   as.data.frame() %>%
                                   select(-width, -strand) %>%
                                   makeGRangesFromDataFrame(keep.extra.columns = TRUE, ignore.strand = TRUE, seqinfo = seqinfo(BSgenome.Hsapiens.UCSC.hg38)) %>%
                                   keepSeqlevels(const_canonical_chromosomes, pruning.mode = "coarse") %>% 
    ...
    current_tibble <-
        tibble(mark = cemt_table_H3K4me1$mark[i],
               celltype = cemt_table_H3K4me1$celltype[i],
               individual = cemt_table_H3K4me1$individual_number[i],
               ref_depth_mean = ase_treatment_vcf_subsetted$refDepth %>% mean(),
               alt_depth_mean = ase_treatment_vcf_subsetted$altDepth %>% mean())
    
    # Add to tibble
    ase_depths_H3K4me1 <- bind_rows(ase_depths_H3K4me1, current_tibble)
}
